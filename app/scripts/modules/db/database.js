...
export async function saveLibraryToDB(newFiles) {
  ...
  existingLibRequest.onsuccess = async () => {
    let libraryData = existingLibRequest.result
      ? existingLibRequest.result.data
      : {};
    ...
    for (const folderName of newFoldersFound) {
      if (libraryData[folderName]) {
        const choice = showModal({
          title: "Carpeta duplicada",
          message: `La carpeta "${folderName}" ya existe en tu biblioteca. ¿Deseas reemplazarla o mantenerla?`,
          onConfirm: () => {
            libraryData[folderName] = [];
          },
          onCancel: () => {
            console.log(`Manteniendo la carpeta "${folderName}" existente.`);
          },
        });
    ...
    const validFiles = newFiles.filter((f) =>
      f.name.match(/\.(mp3|wav|ogg|m4a)$/i),
    );
    */
    ...
    validFiles.forEach((file) => {
      ...
      const isDuplicate = libraryData[folder].some(
